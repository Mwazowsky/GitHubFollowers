...
extension Requestable {
    func url(with config: NetworkConfigurable) throws -> URL {
        ...
        let endpoint: String = isFullPath ? path : baseURL.appending(path)
        
        guard var urlComponents = URLComponents(string: endpoint) else {
            throw RequestGenerationError.components
        }
        
        var urlQueryItems: [URLQueryItem] = [URLQueryItem]()
        
        let queryParametersString = try queryParametersEncodable?.toDictionary() ?? self.queryParameters
