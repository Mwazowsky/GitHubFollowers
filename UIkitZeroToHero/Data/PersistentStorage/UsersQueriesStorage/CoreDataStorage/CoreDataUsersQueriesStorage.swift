...
extension CoreDataUsersQueriesStorage: UsersQueriesStorage {
    ...
    func saveRecentQuery(query: UserQuery, completion: @escaping (Result<UserQuery, any Error>) -> Void) {
...
extension CoreDataUsersQueriesStorage {
    ...
    private func removeDuplicates(
        for query: UserQuery,
        in queries: inout [UserQueryEntity],
        inContext context: NSManagedObjectContext
    ) {
        queries.filter{ $0.query == query.query }
